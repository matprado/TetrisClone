...
const INITIAL_POS = Vector2(144, -16)
...
#Método ready executado quando o Node I é carregado
func _ready():
...
func _process(delta):
	...
	if Input.is_action_just_pressed("Change"):	
		rotate(PI/2)  #gira em 90 graus
		for mino in Minos:
			mino.rotate(-PI/2)
		if not can_move(Vector2(0,0)) or will_collide(Vector2(0,0)):
			#significa que não pode girar, então volta ao estado antetior			
			rotate(-PI/2)
			for mino in Minos:
				mino.rotate(PI/2)
			
	if Input.is_action_pressed("Left") or Input.is_action_pressed("Right") or Input.is_action_pressed("Down"):
		#Se o player está apertando algum botão de controle de movimento
